...
def train_step(X_batch, y_batch, sess, model, summary_op, summary_writer):
    feed_dict = {
...
def train_sentiment(X_train, y_train, eval_data, mode='train'):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            vdcnn = VDCNN(X_train, y_train, num_classes=FLAGS.n_sentiment_classes, embedding_size=FLAGS.embedding_size, l2_reg_weight_decay=FLAGS.l2_weight_decay, mode='train')
            vdcnn.build_graph()

            print("[*] Writing to %s\n" % FLAGS.output_dir)
            train_summary_op = tf.summary.merge([vdcnn.grad_summaries, vdcnn.acc_summary, vdcnn.loss_summary])
            train_summary_dir = os.path.join(FLAGS.output_dir, "sentiment", "summaries", "train")
            train_summary_writer = tf.summary.FileWriter(train_summary_dir, sess.graph)

            dev_summary_op = tf.summary.merge([vdcnn.loss_summary, vdcnn.acc_summary])
            ...
            for batch in batches:
                ...
                train_step(X_batch, y_batch, sess, vdcnn, train_summary_op, train_summary_writer)
                current_step = tf.train.global_step(sess, vdcnn.global_step)

                if current_step % FLAGS.evaluate_every == 0:
                    print("\nSentiment Evaluation:")
                    do_eval(eval_data, sess, vdcnn, dev_summary_op, dev_summary_writer)
...
def train_comment(X_train, y_train, eval_data, mode='train'):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            vdcnn = VDCNN(X_train, y_train, num_classes=FLAGS.n_comment_classes, embedding_size=FLAGS.embedding_size, l2_reg_weight_decay=FLAGS.l2_weight_decay, mode='train')
            vdcnn.build_graph()
...
def inferrence(test_file, max_feature_length):
    comments = []
    sentiments = []
    comment_classes = []
    ...
    for i in xrange(len(comments)):
        ...
        _infer_sentiment(X, y_sentiment)
        # _infer_comment(X, y_comment)


def _infer_sentiment(X, y):
    with tf.Graph().as_default():
        sess = tf.Session()

        with sess.as_default():
            vdcnn = VDCNN(X, y, num_classes=FLAGS.n_sentiment_classes, embedding_size=FLAGS.embedding_size, l2_reg_weight_decay=FLAGS.l2_weight_decay, mode='infer')
            vdcnn.build_graph()
            ...
            prediction = sess.run([vdcnn.predictions], feed_dict={vdcnn.X_input: X, vdcnn.y_input: y})[0]
            ...
            print("[*] SENTIMENT Y_TRUE: %d\n" % y_true)
            print("[*] SENTIMENT PREDICTION: %d" % pred)


def _infer_comment(X, y):
    with tf.Graph().as_default():
        sess = tf.Session()

        with sess.as_default():
            vdcnn2 = VDCNN(X, y, num_classes=FLAGS.n_comment_classes, embedding_size=FLAGS.embedding_size, l2_reg_weight_decay=FLAGS.l2_weight_decay, mode='infer')
            vdcnn2.build_graph()
            ...
            prediction = sess.run([vdcnn.predictions], feed_dict={vdcnn2.X_input: X, vdcnn2.y_input: y})[0]
            pred = np.argmax(prediction, axis=1)
            y_true = np.argmax(y, axis=1)

            print("[*] COMMENT Y_TRUE: %d\n" % y_true)
            print("[*] COMMENT PREDICTION: %d" % pred)
...
def main(_):
    if FLAGS.train:
        ...
        train_sentiment(X_train, y_train_sentiment, sentiment_eval_dict)

        # comment_eval_dict = {'X_dev': X_dev, 'y_dev': y_dev_comment}
        # train_comment(X_train, y_train_comment, comment_eval_dict)
    elif FLAGS.infer:
        inferrence(FLAGS.test_data, FLAGS.max_feature_length)
    else:
        pass
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--test_data',
        type=str,
        default='/Users/Michaeliu/Twitch/VDCNN/test.txt',
        help='Specify test data path'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
        default='/Users/Michaeliu/Twitch/VDCNN/TRAIN_MODEL',
